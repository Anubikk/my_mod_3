...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    if (req.method === "POST" && req.url === "/dog") {
      ...
      dog = new Dog({name, color, age, description}); //encase the req.body in curly braces to make it an object or you will get UNDEFINED
      res.statusCode = 302;
      res.setHeader("Location", "/");
      return res.end();
    }
    ...
    //FIRST ATTMEPT
    // if (req.method === "POST" && req.url === "/cat") {
    //   cat = new Cat(req.body.name, req.body.pattern, req.body.size, req.body.description);
    //   res.statusCode = 302;
    //   res.setHeader("Location", "/");
    //   return res.end(cat);

    // } else if (req.method === "POST" && req.url === "/dog") {
    //   dog = new Dog(req.body.name, req.body.color, req.body.age, req.body.description);
    //   res.statusCode = 302;
    //   res.setHeader("Location", "/");
    //   return res.end(dog);
