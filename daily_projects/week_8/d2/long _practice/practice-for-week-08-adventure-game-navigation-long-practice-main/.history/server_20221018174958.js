...
// Complete the status code, headers, etc.
   ...
   if(req.method === 'GET' && req.url === '/') { // If the request is a GET request and the url is '/'
      ...
      if(view) { // If the view exists
        res.writeHead(200, { // Write the headers
          'Content-Type': 'text/html; charset=utf-8'
        });
        res.write(view.toString().replace('availableRooms', world.availableRoomsToString())); // Write the view to the response
        res.end(); // End the response
      } else { // If the view doesn't exist
        res.writeHead(404); // Write the headers
