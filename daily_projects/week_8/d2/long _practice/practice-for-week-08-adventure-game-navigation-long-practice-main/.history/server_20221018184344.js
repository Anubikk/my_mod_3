    ...
    if(req.method === 'GET' && req.url.startsWith('/rooms/')) {
      ...
      if(roomId !== player.roomId) {
        ...
        const viewWithRooms = viewString
          .replace(/#{roomId}/g, player.roomId)
          .replace(/#{roomName}/g, player.room.name)
          .replace(/#{roomDescription}/g, player.room.description)
          .replace(/#{roomExits}/g, player.room.exitsToString())
          .replace(/#{roomItems}/g, player.room.itemsToString())
          .replace(/#{playerName}/g, player.name)
          .replace(/#{playerHealth}/g, player.health)
          .replace(/#{playerInventory}/g, player.inventoryToString());
        res.statusCode = 200;
        res.setHeader('Content-Type', 'text/html');
        return res.end(viewWithRooms);
      }
    }
