...
// Complete the status code, headers, etc.
   ...
   if(req.method === 'GET' && req.url === '/') { // If the request is a GET request and the url is '/'
      ...
      if(view) { // If the view exists
        ...
        const viewWithRooms = viewString.replace('{{availableRooms}}', availableRooms); // Replace the availableRooms variable in the view with the available rooms

        res.setHeader('Content-Type', 'text/html'); // Set the header to text/html
        res.statusCode = 200; // Set the status code to 200
        res.write(viewWithRooms); // Write the view with the available rooms to the response
