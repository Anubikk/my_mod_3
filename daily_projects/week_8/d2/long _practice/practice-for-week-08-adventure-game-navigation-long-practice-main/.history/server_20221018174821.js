...
// Complete the status code, headers, etc.
   ...
   if(req.method === 'GET' && req.url === '/') { // If the request is a GET request and the url is '/'
      ...
      viewNew = view.replace('{{availableRooms}}', world.availableRoomsToString()); // Replace the availableRooms variable with the string returned from the availableRoomsToString() instance method
      res.setHeader( 'Content-Type', 'text/html' ); // Set the header to text/html
      res.statusCode = 200; // Set the status code to 200
      res.write(viewNew); // Write the view to the response
