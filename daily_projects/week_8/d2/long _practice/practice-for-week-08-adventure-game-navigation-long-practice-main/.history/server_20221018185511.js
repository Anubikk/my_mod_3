...
// room.html view page as the response.
   ...
   if(!player) {
    ...
    return res.end();
   } else if(req.method === 'GET' && req.url === '/rooms/roomId') {
    const currentRoomId = req.url.split('/')[2];
    if(currentRoomId !== player.roomId) {
      res.statusCode = 302;
      res.setHeader('Location', `/rooms/${player.roomId}`);
      return res.end();
    } else {
      ...
      const viewWithRooms = viewString
        ...
        .replace(/#{roomName}/g, player.roomName)
        .replace(/#{roomItems}/g, player.roomItemsToString())
        .replace(/#{roomExits}/g, player.roomExitsToString())
