...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => { // finished assembling the entire request body
    ...
    if (req.method === "GET" && req.url.startsWith("/artists/")) {
      ...
      const artist = artists.filter( artist => artist.artistId === artistId)[0];
      if (artist) {
        res.statusCode = 200;
        res.setHeader("Content-Type", "application/json");
    ...
    if (req.method === "GET" && req.url.startsWith("/albums/")) {
      const albumId = Number(req.url.split("/")[2]); //get the albumId from the url by splitting the url and getting the last element
    ...
    if (req.method === "POST" && req.url.startsWith("/artists/")) {
      ...
      if (artist) {
        ...
        res.statusCode = 201; //status code for created resource successfully
        res.setHeader("Content-Type", "application/json");
    ...
    if (req.method === "DELETE" && req.url.startsWith("/albums/")) {
      ...
      if (album) {
        ...
        res.statusCode = 204; //status code for deleted resource successfully
        return res.end();
      }
    }
    ...
    if (req.method === "GET" && req.url.startsWith("/albums/")) {
      ...
      if (album) {
        const albumSongs = songs.filter((song) => song.albumId === albumId);
        res.statusCode = 200; //status code 200 for success
        res.setHeader("Content-Type", "application/json");
