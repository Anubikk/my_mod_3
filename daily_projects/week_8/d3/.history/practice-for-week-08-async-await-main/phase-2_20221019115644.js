...
// refactor this function to handle Promises using async/await instead of
  // .then and .catch
//   you must stretch for 1 second
// you must run on treadmill for 0.5 seconds
// you must lift weights for 2 seconds

async function workout(totalTime) {
  try {
   await stretch(totalTime);
    await runOnTreadmill(timeLeftAfterStretching);
    await liftWeights(timeLeftAfterRunningOnTreadmill);
}
